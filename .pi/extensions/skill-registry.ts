...
function renderRegistry(cwd: string, sources: string[], entries: SkillEntry[]): string {
	...
	lines.push("");
	lines.push("<!-- Auto-generated by Arc Agent extensions/skill-registry.ts. Run /skill-registry:refresh to regenerate. -->");
	lines.push("");
...
async function ensureArcIgnored(cwd: string): Promise<void> {
	const gitignorePath = join(cwd, ".gitignore");
	let existing = "";
	if (await pathExists(gitignorePath)) {
		existing = await readFile(gitignorePath, "utf8");
	}
	const hasArcIgnore = existing
		.split("\n")
		.map((line) => line.trim())
		.some((line) => line === ".arc" || line === ARC_IGNORE_ENTRY);
	if (hasArcIgnore) return;
	const prefix = existing.length > 0 && !existing.endsWith("\n") ? "\n" : "";
	const header = existing.includes("# Local Arc Agent runtime state")
		? ""
		: "# Local Arc Agent runtime state\n";
	await writeFile(gitignorePath, `${existing}${prefix}${header}${ARC_IGNORE_ENTRY}\n`);
}
...
async function regenerateRegistry(
	...
	await mkdir(join(cwd, ".arc"), { recursive: true });
...
function shouldSkipDuplicateExtensionLoad(
	...
	const projectLocalPath = comparablePath(
		join(cwd, ".pi", "extensions", "skill-registry.ts"),
	);
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag(NO_SKILL_REGISTRY_FLAG, {
		description: "Skip the Arc Agent skill registry refresh and watcher on startup.",
		type: "boolean",
		default: false,
	});
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		try {
			await ensureArcIgnored(ctx.cwd);
			const result = await regenerateRegistry(ctx.cwd, false);
	...
	pi.registerCommand("skill-registry:refresh", {
		description: "Regenerate .arc/skill-registry.md from local skill sources.",
